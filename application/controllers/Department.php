<?php
defined('BASEPATH') OR exit('No direct script access allowed');

class Department extends CI_Controller {

    private $user_session;
	function __construct() {
        parent::__construct();
        $this->common->check_user_session();
		$this->user_session= $this->session->userdata('owner');
        $this->sessionId=$this->session->userdata('owner')['id'];
         
    }
	public function view()
	{
        $data["departments"] = $this->db_model->get_all(TABLE_DEPARTMENT,["is_active"=>1]);
        $this->common->load_view("department/view",$data);
	}

	public function add($college_id = null) {
        $post = $this->input->post();
        if($post){
            $this->form_validation->set_rules('name', 'Department Name', 'trim|required|min_length[1]|max_length[255]'); 
            if ($this->form_validation->run() == FALSE) { 
				$this->session->set_flashdata('message',array("danger",validation_errors())); 
                return  redirect('user/view/'.$college_id);
            } else {
                $data = array(
                    'name' => $this->input->post('name'),
                    'created_by' => $this->user_session['id'],
                    "is_active"=>1,
                    "college_id"=>$college_id
                ); 
                if ($this->db_model->insert(TABLE_DEPARTMENT,$data)) {
                    $this->session->set_flashdata('message', array('success',"Department Created successfully!")); 	
                } else {
                    $this->session->set_flashdata('ma', array('danger',"Failed to create Department."));
                }
                redirect('user/view/'.$college_id);
            }
        }else{
            $this->common->load_view('department/add',['college_id'=> $college_id]);
        }  
	}

	public function edit($id) {
        $post = $this->input->post();
        if($post){
            $this->form_validation->set_rules('name', 'Department Name', 'trim|min_length[3]|max_length[255]'); 
            if ($this->form_validation->run() == FALSE) { 
				$this->session->set_flashdata('message',array("danger",validation_errors())); 
                return redirect('department/add');
            } else {
                $data = array(
                    'name' => $this->input->post('name'),
                    'updated_by' => $this->user_session['id']
                ); 
				if ($this->db_model->update(TABLE_DEPARTMENT,$data,["id"=>$id])) {
                    $this->session->set_flashdata('message', array('success',"Department Updated successfully!")); 	
                } else {
                    $this->session->set_flashdata('ma', array('danger',"Failed to create Department."));
                }
                $response = $this->db_model->get_row(TABLE_DEPARTMENT,["id"=>$id],'college_id' );
                redirect('user/view/'.$response['college_id']);
            }
        }else{
            $data["department"] = $this->db_model->get_row(TABLE_DEPARTMENT,["id"=>$id,"is_active"=>1]); 
            $this->common->load_view('department/add',$data);
        }  
	}

    public function delete($id){  
		$result=$this->db_model->update(TABLE_DEPARTMENT,["is_active"=>0],["id"=>$id]);
		$message = array('success',"Department Deleted SuccessFully");
		if(!$result){
			$message = array('danger',"Something went wrong");
		}
		$this->session->set_flashdata('message', $message); 
		redirect(base_url("department/view"));
	} 
}
