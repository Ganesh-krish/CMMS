...
<div class="layout-content">
    <div class="container-fluid flex-grow-1 container-p-y">
        ...
        <div class="card p-4">
            <form action="<?= isset($course) ? base_url($url . "/course/edit/" . $college_id . '/' . $course['id']) : base_url($url . "/course/add") ?>" method="post">
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="course_code">Course Code</label>
                            <input type="text" class="form-control" id="course_code" name="course_code" value="<?= isset($course['course_code']) ? $course['course_code'] : '' ?>" required>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="course_name">Course Name</label>
                            <input type="text" class="form-control" id="course_name" name="course_name" value="<?= isset($course['name']) ? $course['name'] : '' ?>" required>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" id="description" name="description" rows="3" required><?= isset($course['description']) ? $course['description'] : '' ?></textarea>
                </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control" id="start_date" name="start_date" 
                                value="<?php echo (isset($course) && isset($course['start_date']) && !empty($course['start_date'])) 
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control" id="end_date" name="end_date" 
                                value="<?= (isset($course) && isset($course['end_date']) && !empty($course['end_date'])) 
                                    ? date('Y-m-d', strtotime($course['end_date'])) 
                                    : date('Y-m-d', strtotime('+6 months')) ?>" required>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2-tags" id="tags" name="tags[]" multiple="multiple">
                                    ...
                                    <?php foreach($tags as $tag): ?>
                                        <option value="<?= $tag ?>" <?= (isset($course) && strpos($course['tag'] ?? '', $tag) !== false) ? 'selected' : '' ?>>
                                                    <?= $tag ?>
                                                </option>
                                    <?php endforeach; ?>
                                
                            </select>
                            <small class="form-text text-muted">Type to create new tags or select existing ones</small>
                        </div>
                    </div>
                </div>
                ...
                <div class="text-center mt-4">
                    ...
                    <a href="<?= base_url($url . '/course') ?>" class="btn btn-secondary">Close</a>
                </div>
            </form>
        </div>
    </div>
